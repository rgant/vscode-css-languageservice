...
- **doCodeActions** evaluates code actions for the given location, typically to
  fix a problem.
...
- run VSCode out of sources setup as described here: [How to Contribute](https://github.com/Microsoft/vscode/wiki/How-to-Contribute)
- run `npm link` in the folder of `vscode-css-languageservice`
- use `npm link vscode-css-languageservice` in `vscode/extensions/css-language-features/server`
...
- in VSCode window that is open on the `vscode-css-languageservice` sources, run
  command `Debug: Attach to Node process` and pick the `code-oss` process with
  the `css-language-features` path
...
[`web-data`](https://github.com/microsoft/vscode-custom-data/tree/master/web-data)
and transpiled here. For adding new property or fixing existing properties'
